...
{
    public class Supplier_ContractRepository : Repository<Supplier_Contract, Supplier_ContractViewModel, EShopContext>, ISupplier_ContractRepository
    {
        public Supplier_ContractRepository(IUnitOfWork<EShopContext> unitOfWork, IMapper mappingEngine) : base(unitOfWork, mappingEngine)
        {
        }

        public async Task<Result<PaginatedViewModel<Supplier_ContractViewModel>>> GetPaginatedResult(string? title = null, int take = 10, int skip = 0)
        {
            var result = new Result<PaginatedViewModel<Supplier_ContractViewModel>> ();
