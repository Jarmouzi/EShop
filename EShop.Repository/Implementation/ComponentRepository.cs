...
namespace EShop.Repository.Implementation
{
    public class ComponentRepository : Repository<Component, ComponentViewModel, EShopContext>, IComponentRepository
    {
        ...
        public async Task<Result<PaginatedViewModel<ComponentViewModel>>> GetPaginatedResult(string? title = null, int take = 10, int skip = 0)
        {
            var result = new Result<PaginatedViewModel<ComponentViewModel>> ();
            ...
            try
            {
                ...
                var r = await GetPrecedureAsync<ComponentViewModel>("Component_Get", sparam);

                if(r.Status == TS.Status.Success) {
                    result.Data = new PaginatedViewModel<ComponentViewModel>
                    {
                        Data = r.Data,
