...
namespace EShop.Repository.Implementation
{
    public class BrandRepository : Repository<Brand, BrandViewModel, EShopContext>, IBrandRepository
    {
        ...
        public async Task<Result<PaginatedViewModel<BrandViewModel>>> GetPaginatedResult(string? title = null, int take = 10, int skip = 0)
        {
            var result = new Result<PaginatedViewModel<BrandViewModel>> ();
            ...
            try
            {
                ...
                var r = await GetPrecedureAsync<BrandViewModel>("Brand_Get", sparam);

                if(r.Status == TS.Status.Success) {
                    result.Data = new PaginatedViewModel<BrandViewModel>
                    {
                        Data = r.Data,
