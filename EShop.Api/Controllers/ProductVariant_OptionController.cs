...
namespace EShop.Web.API.Controllers
{
    ...
    public class ProductVariant_OptionController : ControllerBase
    {
        ...
        //{try{var result = await _ProductVariant_OptionRepository.AddAsync(model); return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return BadRequest(ex);}
        //}
        //[HttpPut("Update")]public async Task<IActionResult> Update(ProductVariant_OptionViewModel model)
        //{try{var result = await _ProductVariant_OptionRepository.UpdateAsync(model);return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return BadRequest(ex);}
        //}
        //[HttpDelete("Delete")]
        //public async Task<IActionResult> Delete(Int64 id)
        //{try {var result = await _ProductVariant_OptionRepository.DeleteAsync(id); return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status }); }
        // catch (Exception ex){return BadRequest(ex);}
        //}
        [HttpGet("Get")]
