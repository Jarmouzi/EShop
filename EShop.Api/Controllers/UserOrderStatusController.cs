...
namespace EShop.Web.API.Controllers
{
    ...
    public class UserOrderStatusController : ControllerBase
    {
        ...
        //{try{var result = await _UserOrderStatusRepository.AddAsync(model); return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return BadRequest(ex);}
        //}
        //[HttpPut("Update")]public async Task<IActionResult> Update(UserOrderStatusViewModel model)
        //{try{var result = await _UserOrderStatusRepository.UpdateAsync(model);return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return BadRequest(ex);}
        //}
        //[HttpDelete("Delete")]
        //public async Task<IActionResult> Delete(Int64 id)
        //{try {var result = await _UserOrderStatusRepository.DeleteAsync(id); return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status }); }
        // catch (Exception ex){return BadRequest(ex);}
        //}
        [HttpGet("Get")]
