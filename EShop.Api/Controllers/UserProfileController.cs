...
namespace EShop.Web.API.Controllers
{
    ...
    public class UserProfileController : ControllerBase
    {
        ...
        //[HttpPost("Add")]
        //public async Task<IActionResult> Insert(UserProfileViewModel model)
        //{try{var result = await _UserProfileRepository.AddAsync(model); return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return Ok( new { Message = ex.Message, Status = "server-error" });}
        //}
        //[HttpPut("Update")]public async Task<IActionResult> Update(UserProfileViewModel model)
        //{try{var result = await _UserProfileRepository.UpdateAsync(model);return Ok( new { Data = result.Data, Message = result.Message, Status = result.Status });}
        // catch (Exception ex){return Ok( new { Message = ex.Message, Status = "server-error" });}
        //}
        //[HttpDelete("Delete")]
